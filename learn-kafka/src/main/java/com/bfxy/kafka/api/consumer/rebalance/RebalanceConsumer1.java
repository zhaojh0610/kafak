    ...
    public static void main(String[] args) {
        ...
        consumer.subscribe(Collections.singletonList(Const.TOPIC_REBALANCE), new ConsumerRebalanceListener() {
            ...
            public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
                System.out.println("Revoked Partition:" + partitions);
            }

            @Override
            public void onPartitionsAssigned(Collection<TopicPartition> partitions) {
                System.out.println("Assigned Partition:" + partitions);
            }
        });
        System.out.println(String.format("rebalance1 consumer started..."));
        try {
            //  采用拉取消息的方式消费数据
            while (true) {
                //  等待多久拉取一次消息
